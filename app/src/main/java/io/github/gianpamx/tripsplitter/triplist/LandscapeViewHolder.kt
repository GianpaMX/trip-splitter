...
import kotlinx.android.synthetic.main.trip_list_item_landscape.view.*

class LandscapeViewHolder(val context: Context, parent: ViewGroup?, listener: ((Int) -> Unit)?) : TripListAdapter.ViewHolder(LayoutInflater.from(context).inflate(R.layout.trip_list_item_landscape, parent, false), listener) {
    override fun bind(tripListItem: TripListItem) {
        ...
        builder.setSpan(StyleSpan(Typeface.BOLD), 0, tripListItem.name.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
